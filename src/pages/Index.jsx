    ...
    <Container maxW="container.xl" p={0}>
      ...
      <Flex as="nav" bg="blue.600" color="white" p={4} justifyContent="space-between" alignItems="center">
        ...
        <HStack spacing={8}>
          ...
          <Link as={RouterLink} to="/create-event" color="white">Create Event</Link>
        </HStack>
      </Flex>
      ...
      <Box bg="blue.500" color="white" py={20} textAlign="center">
        ...
        <Button as={RouterLink} to="/create-event" colorScheme="teal" size="lg">Create an Event</Button>
      </Box>
      ...
      <Box py={20} textAlign="center">
        ...
        <VStack spacing={8}>
          {events.map((event, index) => (
            <Box key={index} p={5} shadow="md" borderWidth="1px" width="100%" maxW="container.md">
              <Heading fontSize="xl">{event.eventName}</Heading>
              <Text mt={4}>{event.date} at {event.time}</Text>
              <Text mt={4}>{event.location}</Text>
              <Text mt={4}>{event.description}</Text>
              {event.image && <img src={URL.createObjectURL(event.image)} alt={event.eventName} />}
            </Box>
          ))}
        </VStack>
      </Box>
